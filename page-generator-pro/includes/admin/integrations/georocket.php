...
class Page_Generator_Pro_Georocket extends Page_Generator_Pro_API {
	...
	public function georocket() {
		...
		switch ( $api_call ) {
			...
			default:
                ...
                // If we bypassed the GeoRocket license check, return a permissive
                // response instead of a hard failure so the UI can continue to load.
                if ( $this->is_license_bypass_response( $terms ) ) {
	 ...
	 */
        ...
        /**
         * Determines if the response was generated by bypassing a GeoRocket license
         * check.
         *
         * @param   object $response Response object.
         * @return  bool
         */
        protected function is_license_bypass_response( $response ) {
                return ( is_object( $response ) && isset( $response->source ) && $response->source === 'license_bypass' );
        }
	...
	private function generate_locations( $params ) {
		...
		do {
			...
			$terms = $this->get_cities( $params );
                ...
                // If the request failed due to a missing GeoRocket license, allow the
                // process to continue with an empty result set so the wizard can load
